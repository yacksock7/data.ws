...
class WorkersAssignmentSelectBarUpload extends Component {
    constructor(props) {
        super(props);
        this.state = {
            search: '',
        ...
        if (this.state.assignCount > 0) {
            this.state.assigns.forEach( assign => {
                if (assign.type === AssignType.User) {
                    this.props.workUploadUserStore.addSelectedWorkUploadUser(assign.obj);
                } else if (assign.type === AssignType.Group) {
                    assign.obj.users.forEach( user => {
                        this.props.workUploadUserStore.addSelectedWorkUploadUser(user);
                    });
                }
            });
        }
        ...
        return (
            <div className={classes.root}>
                <Box className={classes.LeftBox}>
                    <UserIcon/>
                    <Typography>작업자 <span>{selectedWorkUploadUsers.length}명</span></Typography>
                </Box>
                            ...
                            <Box className={classes.listBox}>
                                {searchedGroups && searchedGroups.map(group => (
                                    <Box key={`group_${group.id}`} className={classes.listBoxIn}>
                                        ...
                                        <Box style={{marginLeft:10}}>
                                            <Typography className={classes.userIdText} noWrap>{group.name}</Typography>

                                            <Box display='flex' alignItems='center'>
                                                <Typography className={classes.infoText}>{group.createdDatetime} • {group.users.length}</Typography>
                                                <Button className={classes.moreButton} aria-owns={groupOpen ? 'simple-popper' : undefined} onClick={this.handleClickGroupPopover} disableRipple>더보기</Button>
                                                <Popover
                                                    id="simple-popper"
                                                    open={groupOpen}
                                                    ...
                                                    <Box className={classes.groupUserListScroll}>
                                                        {group.users.map((user, i) => (
                                                            <Box key={`Text-User-${user.id}`} className={classes.groupUserListIn}>
                                                                <UserIcon/>
                                                                <Box>
                                                                    <Typography className={classes.groupUserListInText}>{user.nickname}</Typography>
                                                                    <Typography className={classes.groupUserListInText2}>{user.email} • {user.createdDatetime} 가입</Typography>
                                                                </Box>
                                                            </Box>
                                                        ))}
                                                    </Box>
                                ...
                                {searchedUsers.map(user => (
                                    <Box key={`user_${user.id}`} className={classes.listBoxIn}>
                                        ...
                                        <UserIcon/>
                                        <Box style={{marginLeft:10}}>
                                            <Typography className={classes.userIdText} noWrap>{user.nickname}</Typography>
                                            <Box display='flex' alignItems='center'>

                                                <Typography className={classes.infoText} noWrap style={{maxWidth: 195}}>
                                                    {user.email} {(user.groups && user.groups.length > 0) && " • " + user.groups[0].name}
                                                </Typography>
                                                {user.groups && user.groups.length > 1 &&
                                                    <>
                                                        <Button className={classes.moreButton} onClick={this.handleClickPersonPopover} disableRipple>더보기</Button>
                                                        ...
                                                        <Box className={classes.groupUserListScroll} style={{margin: '8px 12px'}}>
                                                            {user.groups.map(group => {
                                                                return <Typography key={`Text-Group-${group.id}`} className={classes.groupUserListInText}>{group.name} 작업 그룹</Typography>
                                                            })}
                                                        </Box>
                                                        </Popover>
                                                    </>
...
export default withStyles(styles) (
    inject('userStore', 'workUploadUserStore') (
        observer(WorkersAssignmentSelectBarUpload)
    )
);